...
class FakeIsolatedMotion:
    """Заглушка координатора для контроллера L3."""
    ...
    def set_l3_goal(self, target: TargetPoint) -> L3State:
        """Сохранить цель L3."""
        self.goal_calls.append(target)
        return self.state

    def set_l3_route(self, route: TargetRoute) -> L3State:
...
def test_l3_goal_route_step_and_cancel_forward_commands() -> None:
    """REST-обработчики L3 передают цель, маршрут, шаг и отмену."""
    ...
    async def run() -> None:
        ...
        assert isolated_motion.goal_calls[0] == TargetPoint(x_cm=10.0, y_cm=20.0)
        assert isolated_motion.route_calls[0] == TargetRoute(
...
def test_l3_ws_sends_state_snapshot() -> None:
    """WebSocket L3 отправляет состояние и завершается после отключения."""
