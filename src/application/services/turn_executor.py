...
class TurnExecutor:
    ...
    def run(
        ...
        *,
        ...
        self._run_turn_segment(turn_left, target_angle, timeout_sec)

    def _run_turn_segment(self, turn_left: bool, target_angle: float, timeout_sec: float) -> None:
        """Выполнить один поворотный цикл до цели, таймаута или препятствия."""
        ...
        while self.lifecycle.should_keep_running():
            ...
            if (now - last_obstacle_check_t) >= self.turn_obstacle_check_interval_sec:
                if self.ultrasonic_sensor.measure_distance_cm() <= self.min_obstacle_distance_cm:
                    break
                last_obstacle_check_t: float = now
