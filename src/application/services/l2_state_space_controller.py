...
class L2StateSpaceController:
    """LQR-контроллер МПС для L2.

    Вычисляет корректировки управления ``v_cmd`` и ``omega_cmd_rad_s`` по
    линеаризованной модели дифференциального привода.
    """
    ...
    def compute_gains(self, v0: float) -> np.ndarray:
        ...
        # Оптимальная матрица K = R^-1 B^T P.
    ...
    def compute_control(
        ...
        *,
        ...
        # Пересчитать матрицу K, если базовая скорость заметно изменилась.
